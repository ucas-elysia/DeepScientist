...
test.describe('onboarding launch flow', () => {
  test('step 2 can advance into the launch-mode dialog', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Open the launch dialog' })).toBeVisible()

    await page.getByRole('button', { name: 'Next' }).click()

    await expect(page.locator('[data-onboarding-id="experiment-launch-dialog"]')).toBeVisible({ timeout: 30_000 })
    await expect(page.getByText('3 / 29')).toBeVisible({ timeout: 30_000 })
  ...
  test('step 19 keeps a connected canvas instead of isolated nodes', async ({ page }) => {
    await page.setViewportSize({ width: 1600, height: 1000 })
    await openProjectTutorial(page, 'en')

    for (let attempt = 0; attempt < 20; attempt += 1) {
      const bodyText = await page.locator('body').innerText()
      const match = bodyText.match(/STEP\s+(\d+)\s*\/\s*29|Step\s+(\d+)\s*\/\s*29/)
      const step = match ? Number(match[1] || match[2]) : null
      if (step != null && step >= 19) {
        break
      }
      await page.getByRole('button', { name: 'Next' }).click()
    ...
    await expect(page.getByText('Step 19 / 29')).toBeVisible({ timeout: 30_000 })
